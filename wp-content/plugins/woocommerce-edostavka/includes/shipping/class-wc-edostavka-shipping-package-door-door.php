<?php

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}


class WC_Edostavka_Shipping_Package_Door_Door extends WC_Edostavka_Shipping {

	protected $code = 139;
	
	protected $delivery_type = 'door';
	
	protected $weight_limit = 30;
	
	public function __construct( $instance_id = 0 ) {
		$this->id           		= 'edostavka-package-door-door';
		$this->method_title 		= 'Посылка дверь-дверь (СДЭК)';

		parent::__construct( $instance_id );
		
		$this->method_description 	= 'Услуга экономичной доставки товаров по России для компаний, осуществляющих дистанционную торговлю.';
	}
}